    ...
    stage('Build') {
      steps {
        sh '''echo "Creating Project Package"
          ...
          $APIGWDEPLOYTOOLS/apigateway/posix/bin/projpack --create  --dir=. --passphrase-none --name=common --type=pol --add ${WORKSPACE}"/APIProject11" --projpass-none --add ${WORKSPACE}"/APIProject22" --projpass-none --add ${WORKSPACE}"/GrowSuperAPIProject" --projpass-none --add ${WORKSPACE}"/commonProjectDefault" --projpass-none
